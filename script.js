...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /* ---------- 6. CONTACT FORM ---------- */
  // EDIT: change this to your deployed backend's address once it's hosted
  // (e.g. 'https://api.visvathaschool.com/api/enquiries'). While developing
  // locally with `npm run dev` in /backend, the default below is correct.
  var API_URL = 'http://localhost:5000/api/enquiries';
    ...
    form.addEventListener('submit', function (e) {
      ...
      fetch(API_URL, {
        ...
        .then(function (result) {
          if (!result.ok) {
            var msg = (result.data.errors && result.data.errors.join(' ')) || result.data.error || 'Something went wrong. Please try again.';
            throw new Error(msg);
          }
          status.textContent = result.data.message || 'Thank you! Your enquiry has been received.';
          status.className = 'success';
          form.reset();
        })
        .catch(function (err) {
          // Most likely cause during development: the backend isn't running.
          // Start it with `npm run dev` inside the /backend folder.
          status.textContent = err.message.indexOf('Failed to fetch') !== -1
            ? 'Could not reach the server. Please try again shortly, or contact us directly by phone.'
            : err.message;
          status.className = 'error';
        })
        .finally(function () {
